    ...
    class TruncatedOctahedronQuantizer {
        ...
        auto quantize(std::vector<Eigen::RowVector<T, 3>> const& points) const -> std::vector<Eigen::RowVector3i> {
            ...
            for (const auto& pt : points) {
                ...
                Eigen::RowVector3i base = uvw.array().floor().template cast<int>();
                ...
                for (const auto& offset : offsets) {
                    ...
                    Eigen::RowVector<T, 3> candidate_center = transform_point<T>(candidate_voxel.cast<T>(), get_transform_matrix<T>(TRUNC_OCT_SCALE_));
